...
class SquaredErrorObjective():
    def loss(self, y, pred): return np.mean((1/2)*(y - pred)**2)
...
class XGBoostModel:
    def __init__(self, params, random_seed=None):
        ...
        self.col_subsample = self.params['col_subsample'] \
            if self.params['col_subsample'] else 1.0
    ...
    def fit(self, X, y, objective, num_boost_round, verbose=False):
        ...
        for i in range(num_boost_round):
            ...
            sample_idxs = None if self.subsample == 1.0 \
                                    ...
                                    replace=False)
            sample_column = None if self.col_subsample == 1.0 \
                else self.rng.choice(X.shape[1],
                                    size=math.floor(self.col_subsample*X.shape[1]),
                                    replace=False)
            booster = TreeBooster(X, gradients, hessians, 
                                self.params, self.max_depth, sample_idxs, sample_column)
...
class TreeBooster:
    def __init__(self, X, g, h, params, max_depth, idxs=None, sample_column = None):
        self.params = params
        ...
        self.n = len(idxs)
        self.c = sample_column if sample_column else [i for i in range(X.shape[1])]
        self.value = -g[idxs].sum() / (h[idxs].sum() + self.reg_lambda) # Eq (5)
        self.best_score_so_far = 0.
        if self.max_depth > 0:
            self._maybe_insert_child_nodes()
        self.default_missing = "left"
    
    def _maybe_insert_child_nodes(self):
        for i in self.c: self._find_better_split(i)
        if self.is_leaf: return
        x = self.X.values[self.idxs,self.split_feature_idx]
        left_idx = np.nonzero((x <= self.threshold) | np.isnan(x))[0] \
            if self.direction == "left" else np.nonzero(x <= self.threshold)[0]
        right_idx = np.nonzero((x > self.threshold) | np.isnan(x))[0] \
            if self.direction == "right" else np.nonzero(x > self.threshold)[0]
        self.left = TreeBooster(self.X, self.g, self.h, self.params, 
                                self.max_depth - 1, self.idxs[left_idx], self.c)
        self.right = TreeBooster(self.X, self.g, self.h, self.params, 
                                 self.max_depth - 1, self.idxs[right_idx], self.c)
    ...
    def _find_better_split(self, feature_idx):
        ...
        mask = np.isnan(x)
        sum_g_nan, sum_h_nan = g[mask].sum(), h[mask].sum()
        
        for i in range(0, self.n - mask.sum() - 1):
            g_i, h_i, x_i, x_i_next = sort_g[i], sort_h[i], sort_x[i], sort_x[i + 1]
            if np.isnan(x_i_next):
                break
            sum_g_left += g_i; sum_g_right -= g_i
            sum_h_left += h_i; sum_h_right -= h_i
            if sum_h_left < self.min_child_weight or x_i == x_i_next:continue
            if sum_h_right < self.min_child_weight: break
            self.gain_calculation(sum_g_left + sum_g_nan,
                                  sum_g_right, 
                                  sum_h_left + sum_h_nan, 
                                  sum_h_right, 
                                  sum_g, sum_h,
                                  feature_idx, 
                                  x_i, x_i_next, "left")
            self.gain_calculation(sum_g_left, 
                                  sum_g_right + sum_g_nan, 
                                  sum_h_left, 
                                  sum_h_right + sum_h_nan, 
                                  sum_g, sum_h, 
                                  feature_idx, 
                                  x_i, x_i_next, "right")
    
    def gain_calculation(self, g_left, g_right, h_left, h_right, g, h, feature_idx, x_i, x_i_next, direction):
    ...
    def _predict_row(self, row):
        if self.is_leaf: 
            return self.value
        if np.isnan(row[self.split_feature_idx]):
            child = self.left if self.direction == "left" else self.right
